...
from app.schemas.ai import AIReplyRequest, AISummarizeConversationRequest
from app.services.gemini_service import (
    suggest_whatsapp_reply, extract_lead_from_text, summarize_conversation_history
)
...
async def ai_extract_lead(
    text: str = Query(..., description="Raw text containing lead contact information")
):
...
async def ai_summarize_conversation(
    payload: AISummarizeConversationRequest, db: AsyncSession = Depends(get_db)
):
    res = await db.execute(
        select(WhatsAppMessage)
        .where(WhatsAppMessage.conversation_id == payload.conversation_id)
    ...
    if not messages:
        raise HTTPException(
            status_code=404, detail="No messages found in this conversation"
        )
